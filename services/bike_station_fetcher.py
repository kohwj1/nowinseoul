...
def parking_info(api_data, station_id_dict):
    ...
    if station_id in station_id_dict:
        value = {'SBIKE_SPOT_NM' : station_id_dict.get(station_id), # station_name_ko(대여소 영어 이름)
...
def get_info(attraction_id, my_locale): # POI033 서울역
    ...
    print(f'{len(result_data)}개 대여소 정보가 조회되었습니다.')
    return result_data

if __name__ == "__main__":
    result_bike_station_list = get_info("POI033",'en') # POI033 서울역
