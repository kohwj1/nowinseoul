...
(function renderCrowdBar() {
  ...
  const badgeEl = document.querySelector('.crowd-density-section .info-tag, .crowd-density-section #crowd-tag, .crowd-density-section #traffic-tag');
  ...
  const levelText = (level === 'Moderate') ? 'Medium' : level;
  const cls = level === 'Comfortable' ? 'comfortable'
           : level === 'Moderate'    ? 'medium'
           : level === 'Crowded'     ? 'congested' : '';
  if (badgeEl) {
    badgeEl.textContent = levelText;
    badgeEl.classList && badgeEl.classList.remove('comfortable','medium','congested');
  ...
  const colorOf = (lvl) => (
    lvl === 'Comfortable' ? '#22c55e' :     
    lvl === 'Moderate'    ? '#f59e0b' :      
    lvl === 'Crowded'     ? '#ef4444' : '#a3a3a3' 
  );
